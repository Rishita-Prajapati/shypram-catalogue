...
export default function CartPage() {
  ...
  const { items, cuttingQuantity, removeItem, updateQuantity, updateCuttingQuantity, clearCart, getTotalWeight } = useCartStore()
  ...
  const handleSubmitOrder = async () => {
    ...
    if (!cuttingQuantity || parseInt(cuttingQuantity) <= 0) {
      toast({
        title: "Missing cutting quantity",
        description: "Please enter a valid cutting quantity",
        variant: "destructive"
      })
      return
    }
    ...
    if (!connectionOk) {
      toast({
        ...
        description: "Cannot connect to database. Check console for details.",
        variant: "destructive"
      })
    ...
    try {
      ...
      const orderId = await DatabaseOrderStore.addOrder({
        ...
        orderDate: new Date(),
        notes,
        cuttingQuantity: parseInt(cuttingQuantity) || 0
      })
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Cutting Quantity (Common for all products)</CardTitle>
                </CardHeader>
                <CardContent>
                  <div>
                    <Label htmlFor="cuttingQuantity">Cutting Quantity (kg) *</Label>
                    <Input
                      id="cuttingQuantity"
                      type="number"
                      value={cuttingQuantity}
                      onChange={(e) => updateCuttingQuantity(e.target.value)}
                      placeholder="Enter cutting quantity in kg"
                      min="1"
                      required
                    />
                    <p className="text-sm text-muted-foreground mt-1">
                      This cutting quantity (in kg) will apply to all products in your order
                    </p>
                  </div>
                </CardContent>
              </Card>
