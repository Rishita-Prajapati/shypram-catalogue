...
-- Enable RLS for other sensitive tables
ALTER TABLE admin_users ENABLE ROW LEVEL SECURITY;
ALTER TABLE contact_inquiries ENABLE ROW LEVEL SECURITY;

-- Admin users policy (only authenticated admins can access)
CREATE POLICY "Admin access only" ON admin_users
FOR ALL USING (auth.role() = 'authenticated');

-- Contact inquiries policy (allow insert for anonymous, read for admins)
